...
type serveConfig struct {
	listen    string // TCP address, e.g. "127.0.0.1:8080" (empty = disabled)
	unix      string // Unix socket path (empty = disabled)
	token     string // optional shared secret (Bearer or X-GDCC-Token)
...
// must not expose its state-changing endpoint (/report writes public DCC
// counts) to the network. With no token, only a loopback TCP bind (or a Unix
// socket) is allowed; binding all interfaces or a routable address needs a token.
func (cfg serveConfig) safetyCheck() error {
	if cfg.token != "" || cfg.listen == "" {
		return nil
	}
	if !isLoopbackListen(cfg.listen) {
		return fmt.Errorf("refusing to serve on %s without a token: set --token/GDCC_TOKEN, or bind a loopback address (127.0.0.1)", cfg.listen)
	}
	return nil
}

// isLoopbackListen reports whether a TCP listen address binds only loopback. An
// empty host (":8080") means all interfaces — not loopback.
...
// runServe starts the HTTP daemon: /check, /report (gozer-shaped JSON),
// /metrics (Prometheus), /healthz. It mirrors the in-process library so a
// standalone gdcc can be a DCC sidecar.
func runServe(c *dcc.Client, cfg serveConfig, stdout, stderr io.Writer) int {
	var infoW io.Writer = stderr
	if cfg.logStdout {
		infoW = os.Stdout
	}
	info := log.New(infoW, "gdcc: ", 0)
	errl := log.New(stderr, "gdcc: ", 0)
	...
	m := newMetrics()
	// Bound in-flight requests: each fans out UDP queries to the DCC servers,
	...
	// volume so it logs only under --verbose, /report always.
	...
	mux.HandleFunc("/check", cfg.auth(gate(access("check", m.instrument("check", checkHandler(c, m))))))
	mux.HandleFunc("/report", cfg.auth(gate(access("report", m.instrument("report", reportHandler(c, m))))))

	// WriteTimeout must exceed the DCC client budget so a slow upstream does
	// not get cut off mid-answer; IdleTimeout bounds keep-alive connections.
	srv := &http.Server{
		Handler:           mux,
		ReadHeaderTimeout: 10 * time.Second,
		ReadTimeout:       30 * time.Second,
		WriteTimeout:      c.Timeout + 15*time.Second,
...
// checkResponse is the gozer-compatible /check answer: action + bulk, plus the
// raw per-checksum counts for debugging.
type checkResponse struct {
	Action string      `json:"action"` // reject | accept | unknown
	Bulk   *int        `json:"bulk"`
	Counts []countJSON `json:"counts,omitempty"`
}
...
func checkHandler(c *dcc.Client, m *metrics) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		v := res.Verdict()
		m.verdictInc(v.Action.String())
		out := checkResponse{Action: v.Action.String(), Bulk: v.Bulk}
		for _, ck := range res.Counts {
			out.Counts = append(out.Counts, countJSON{ck.Label, ck.Cur, ck.Prev})
		}
		writeJSON(w, http.StatusOK, out)
	}
}

func reportHandler(c *dcc.Client, m *metrics) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		msg, err := readBody(w, r)
		if err != nil {
			return
...
// X-GDCC-Token header). No token configured → open.
func (cfg serveConfig) auth(next http.HandlerFunc) http.HandlerFunc {
	if cfg.token == "" {
		return next
	}
	return func(w http.ResponseWriter, r *http.Request) {
		got := r.Header.Get("X-GDCC-Token")
		if got == "" {
